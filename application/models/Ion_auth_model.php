...
class Ion_auth_model extends CI_Model
{
	...
	public function clear_forgotten_password_code($code) {
		...
		$this->db->where('kode_lupa_password', $code);

		if ($this->db->count_all_results($this->tables['user']) > 0)
		{
			$data = array(
			    'kode_lupa_password' => NULL,
			    'waktu_lupa_password' => NULL
			);

			$this->db->update($this->tables['user'], $data, array('kode_lupa_password' => $code));
	...
	public function forgotten_password($identity)
	{
		...
		$update = array(
		    'kode_lupa_password' => $key,
		    'waktu_lupa_password' => time()
		);
	...
	public function forgotten_password_complete($code, $salt=FALSE)
	{
		...
		$profile = $this->where('kode_lupa_password', $code)->users()->row(); //pass the code to profile
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$query = $this->db->get_where($this->tables['groups'],array('nama' => $this->config->item('default_group', 'ion_auth')),1)->row();
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, aktif, login_terakhir')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['user']);
		...
		if($this->is_max_login_attempt_exceeded($identity))
		{
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				$this->update_login_terakhir($user->id);
	...
	public function recheck_session()
    {
        ...
        if($recheck!==0)
        {
            $login_terakhir = $this->session->userdata('last_check');
            if($login_terakhir+$recheck < time())
            {
                ...
                else
                {
                    ...
                    {
                        $this->session->unset_userdata( array($identity => '', 'id' => '', 'id_user' => '') );
                    }
                    else
                    {
                        $this->session->unset_userdata( array($identity, 'id', 'id_user') );
	 ...
	 * @return boolean
	 **/
	public function is_max_login_attempt_exceeded($identity, $ip_address = NULL) {
		if ($this->config->item('track_login_attempt', 'ion_auth')) {
			$max_attempts = $this->config->item('maximum_login_attempt', 'ion_auth');
			if ($max_attempts > 0) {
	...
	public function get_attempts_num($identity, $ip_address = NULL)
	{
        if ($this->config->item('track_login_attempt', 'ion_auth')) {
            ...
            $qres = $this->db->get($this->tables['login_attempt']);
	 ...
	 *
	 * This function is only a wrapper for is_max_login_attempt_exceeded() since it
	...
	public function is_time_locked_out($identity, $ip_address = NULL) {
		return $this->is_max_login_attempt_exceeded($identity, $ip_address);
	}
	...
	public function get_last_attempt_time($identity, $ip_address = NULL) {
		if ($this->config->item('track_login_attempt', 'ion_auth')) {
			...
			$qres = $this->db->get($this->tables['login_attempt'], 1);
	...
	*/
	public function get_last_attempt_ip($identity) {
		if ($this->config->item('track_login_attempt', 'ion_auth') && $this->config->item('track_login_ip_address', 'ion_auth')) {
	...
	public function increase_login_attempt($identity) {
		if ($this->config->item('track_login_attempt', 'ion_auth')) {
			...
			return $this->db->insert($this->tables['login_attempt'], $data);
		}
		return FALSE;
	}
	 ...
	 *                            Use get_last_attempt_ip($identity) to retrieve user's last IP
	 **/
	public function clear_login_attempt($identity, $old_attempts_expire_period = 86400, $ip_address = NULL) {
		if ($this->config->item('track_login_attempt', 'ion_auth')) {
			...
			return $this->db->delete($this->tables['login_attempt']);
		}
		return FALSE;
	}
	...
	public function users($groups = NULL)
	{
		...
		else
		{
			...
			$this->db->select(array(
			    ...
			    $this->tables['user'].'.id as id_user'
			));
		}
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				$this->db->distinct();
				$this->db->join(
				    $this->tables['group_user'],
				    $this->tables['group_user'].'.'.$this->join['user'].'='.$this->tables['user'].'.id',
				    'inner'
				);
			}
			...
			foreach($groups as $group)
			{
				if(is_numeric($group)) $id_groups[] = $group;
				else $group_names[] = $group;
			}
			$or_where_in = (!empty($id_groups) && !empty($group_names)) ? 'or_where_in' : 'where_in';
			// if group name was used we do one more join with groups
			if(!empty($group_names))
			{
				$this->db->join($this->tables['groups'], $this->tables['group_user'] . '.' . $this->join['groups'] . ' = ' . $this->tables['groups'] . '.id', 'inner');
				$this->db->where_in($this->tables['groups'] . '.nama', $group_names);
			}
			if(!empty($id_groups))
			{
				$this->db->{$or_where_in}($this->tables['group_user'].'.'.$this->join['groups'], $id_groups);
			}
		}
	...
	public function user($id = NULL)
	{
		...
		$id = isset($id) ? $id : $this->session->userdata('id_user');
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select($this->tables['group_user'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.nama, '.$this->tables['groups'].'.deskripsi')
		                ->where($this->tables['group_user'].'.'.$this->join['user'], $id)
		                ->join($this->tables['groups'], $this->tables['group_user'].'.'.$this->join['groups'].'='.$this->tables['groups'].'.id')
		                ->get($this->tables['group_user']);
	}
	 ...
	 * @author Ben Edmunds
	 **/
	public function add_to_group($id_groups, $id_user=false)
	{
		...
		{
			if ($this->db->insert($this->tables['group_user'], array( $this->join['groups'] => (float)$id_group, $this->join['user'] => (float)$id_user)))
			{
	 ...
	 * @author Ben Edmunds
	 **/
	public function remove_from_group($id_groups=false, $id_user=false)
	{
		...
		if( ! empty($id_groups))
		{
			...
			foreach($id_groups as $id_group)
			{
				$this->db->delete($this->tables['group_user'], array($this->join['groups'] => (float)$id_group, $this->join['user'] => (float)$id_user));
				if (isset($this->_cache_user_in_group[$id_user]) && isset($this->_cache_user_in_group[$id_user][$id_group]))
				{
					unset($this->_cache_user_in_group[$id_user][$id_group]);
				}
			}
		...
		{
			if ($return = $this->db->delete($this->tables['group_user'], array($this->join['user'] => (float)$id_user))) {
	...
	**/
	...
	/**
	 * update_login_terakhir
	 *
	 * @return bool
	 * @author Ben Edmunds
	 **/
	public function update_login_terakhir($id)
	{
		$this->trigger_events('update_login_terakhir');

		$this->load->helper('date');

		$this->trigger_events('extra_where');

		$this->db->update($this->tables['user'], array('login_terakhir' => time()), array('id' => $id));
	...
	public function set_session($user)
	{
		...
		$session_data = array(
		    ...
		    'email'                => $user->email,
		    'id_user'              => $user->id, //everyone likes to overwrite id so we'll use id_user
		    'old_login_terakhir'       => $user->login_terakhir,
		    'last_check'           => time(),
		);
	...
	public function remember_user($id)
	{
		...
		$this->db->update($this->tables['user'], array('kode_pengingat' => $salt), array('id' => $id));
	...
	public function login_remembered_user()
	{
		...
		// get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', id, email, login_terakhir')
		                  ->where($this->identity_column, urldecode(get_cookie($this->config->item('identity_cookie_name', 'ion_auth'))))
		                  ->where('kode_pengingat', get_cookie($this->config->item('remember_cookie_name', 'ion_auth')))
				  ->where('aktif',1)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['user']);
	...
	*/
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
	{
		...
		$id_group = $this->db->insert_id($this->tables['groups'] . '_id_seq');
	 ...
	 * @author aditya menon
	 **/
	public function update_group($id_group = FALSE, $group_name = FALSE, $additional_data = array())
	{
		...
		if (!empty($group_name))
		{
			...
			if(isset($existing_group->id) && $existing_group->id != $id_group)
			{
				$this->set_error('group_already_exists');
				return FALSE;
			}
        ...
        $group = $this->db->get_where($this->tables['groups'], array('id' => $id_group))->row();
		...
		$this->db->update($this->tables['groups'], $data, array('id' => $id_group));
	...
	* @author aditya menon
	**/
	public function delete_group($id_group = FALSE)
	{
		// bail if mandatory param not set
		if(!$id_group || empty($id_group))
		{
			return FALSE;
		}
		$group = $this->group($id_group)->row();
		...
		$this->db->delete($this->tables['group_user'], array($this->join['groups'] => $id_group));
		// remove the group itself
		$this->db->delete($this->tables['groups'], array('id' => $id_group));
